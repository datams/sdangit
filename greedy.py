	...
	##### CONSIDER REROUTE #####
		...
		if path_pack_in_empty_graph == []:
			...
			if len(lp_sel_paths)==len(d_list_subset):
				for index in d_index_subset:
					[G_updated, sel_path]=gf.alloc_p(G_updated, d_list[index], lp_sel_paths[index][0])
					plot_pool.plot(G_updated, d_list[index], 3, plot_enable)
