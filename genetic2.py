...
def paraevolution(G,d_list,pop_size,clergy_size,clergy_children,nobility_size,nobility_children, start_mut, non_prob, weight_ac, mut_method, target_ratio, e, return_ratio, return_paths, ga_thread):
	...
	gf.write2file('GAGACHECK', 'melde ga_thread '+str(ga_thread)+' before feasible paths')
	...
	gf.write2file('GAGACHECK', 'melde ga_thread '+str(ga_thread)+' after feasible paths')
	# create population
	...
	while(True):
		...
		target_hit=False
		if target_ratio-0.02<=acc_ratio:
			target_hit=True
