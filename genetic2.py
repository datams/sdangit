...
def paraevolution(G,d_list,pop_size,clergy_size,clergy_children,nobility_size,nobility_children, start_mut, non_prob, weight_ac, mut_method, target_ratio, e, return_ratio, return_paths, ga_thread):
	...
	gf.write2file('GAGARATE', 'melde ga_thread '+str(time.strftime("%H%M%S"))+str(ga_thread)+' vor feasible paths')
	...
	gf.write2file('GAGARATE', 'melde ga_thread '+str(time.strftime("%H%M%S"))+str(ga_thread)+' nach feasible paths')
