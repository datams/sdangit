...
# get all demands, whose path intersect with optimal path by at least one and are not blocked and would release enough bw
def find_d_to_reroute(optimal_path, bw_req, d_list):
	...
	for i in range(len(d_list)-1): # not compare with the newest (the one that cannot fit) demand
		...
		print 'finde degree '+str(degree)+' fuer demand nummer '+str(i)+' and bw '+str(d_list[i].bw)
		if degree>0 and d_list[i].blocked==False and d_list[i].bw>=bw_req:
