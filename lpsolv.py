...
def solve(G,d_list):
	...
	for r in Requests:
		...
		for v in m.getVars():
			...
			# get all P vars
			# CAVEAT.. STARTWITH
			if varName.startswith('P'+str(r)+'_') and varValue==1:
				P=varName
		...
		if e_list.select(s[r],'*')!=[]: # if there really is a path, continue
			...
			if temp_edge[1]!=t[r]:
				for j in range(1,len(e_list)): # write all other nodes
					if len(e_list.select(pp[j],'*'))<1:
						print 'BUG?!'
						print 'EMPTY MATCHING FOUND...'
						print 'r: '+str(r)
						print 'paths[r, s[r], t[r]] '+str(paths[r, s[r], t[r]])
						print "e_list.select(pp[j],'*'): " +str(e_list)
						print "e_list: " +str(e_list.select(pp[j],'*'))
						print 's[r]: '+str(s[r])
						print 't[r]: '+str(t[r])
						print 'pp up to now '+str(pp)
						print 'now looking for tuples beginning with '+str(pp[j])
