		...
		# otherwise look for rerouting possibilities
			...
			if path_pack_in_empty_graph == []:
				...
				if shortest_intersect!=[]:
					...
					print 'although the shortest_intersect path '+str(shortest_intersect[0])+' ==> '+str(shortest_intersect[-1])+' is: '+str(shortest_intersect)
					...
					print 'demand number: '+str(d_u)+' to the demand from'+str(d_list[d_u].source)+' to '+str(d_list[d_u].target)
